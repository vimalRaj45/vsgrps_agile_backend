...
async function taskRoutes(fastify, options) {
  ...
  fastify.get('/', async (req, reply) => {
    ...
    let query = 'SELECT t.*, u.name as assignee_name, u.avatar_url as assignee_avatar, p.name as project_name FROM tasks t LEFT JOIN users u ON t.assigned_to = u.id LEFT JOIN projects p ON t.project_id = p.id WHERE t.company_id = $1';
  ...
  fastify.get('/:id', async (req, reply) => {
    ...
    const { rows } = await pool.query(
      'SELECT t.*, u.name as assignee_name, u.avatar_url as assignee_avatar, p.name as project_name FROM tasks t LEFT JOIN users u ON t.assigned_to = u.id LEFT JOIN projects p ON t.project_id = p.id WHERE t.id = $1 AND t.company_id = $2',
